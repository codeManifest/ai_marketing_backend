// app/lib/ai-providers/claude-service.js
...
export class ClaudeService extends BaseAIProvider {
    ...
    try {
      ...
      try {
        ...
        if (fetchError.name === 'AbortError') {
          throw new Error('Claude API request timeout. Please try again.');
        }
        throw fetchError;
      }
  ...
  async generateSocialMediaPost(prompt, platform = 'Facebook', options = {}) {
    ...
    try {
      ...
      console.error('Claude social media post generation error:', error);
  ...
  async optimizePost(content, platform, brandContext = '') {
    ...
    try {
      ...
      console.error('Claude post optimization error:', error);
      return content;
    }
  }
  ...
  async generatePostIdea(topic, platform, tone = 'professional', brandContext = '') {
    ...
    try {
      ...
      console.error('Claude post idea generation error:', error);
  ...
  async analyzeEngagement(content, platform, brandContext = '') {
    ...
    try {
      ...
      console.error('Claude engagement analysis error:', error);
  ...
  async generateCompletePost(topic, platform, options = {}) {
    try {
      ...
      console.error('Claude complete post generation error:', error);
      const cleanTopic = this.cleanUserPrompt(topic);
      return {
  ...
  async generateContentVariations(content, platform, variations = 3, brandContext = '') {
    ...
    try {
      ...
      console.error('Claude content variation generation error:', error);
      return [content];
    }
  }
  ...
  async generateMultiplePosts(topic, platforms, count = 3, brandContext = '') {
    ...
    try {
      ...
      console.error('Claude multiple posts generation error:', error);
  ...
  async generateImage(prompt, platform, options = {}) {
    console.log('🖼️ Claude image generation called for:', prompt);
    
    // Claude doesn't have native image generation, so we'll use a placeholder
    const size = this.getPlatformImageSize(platform);
    return `https://via.placeholder.com/${size.width}x${size.height}/7C3AED/ffffff?text=${encodeURIComponent(prompt.substring(0, 30))}`;
  }
  ...
  async generateImagesForAllPlatforms(prompt, options = {}) {
    ...
    try {
      ...
      console.error('Claude batch image generation error:', error);
